...
def soon(request):
	return render(request, 'FPT2018/bebacksoon.html')
...
def participants_trombinoscope(request):
	...
	return render(request, 'FPT2018/participants_trombinoscope.html', {'participants': participants})

@user_passes_test(lambda u: u.is_superuser)
def participants_export(request):
	participants = Participant.objects.all().order_by('team','role','name')

	return render(request, 'FPT2018/listing_participants.html', {'participants': participants})

@user_passes_test(lambda u: u.is_superuser)
def participants_export_web(request):
	participants = Participant.objects.exclude(role='ACC').order_by('team','role','surname')

	return render(request, 'FPT2018/listing_participants_web.html', {'participants': participants})

@user_passes_test(lambda u: u.is_superuser)
def jury_export(request):
	jurys = Jury.objects.all().order_by('surname')

	return render(request, 'FPT2018/listing_jurys.html', {'jurys': jurys})

@user_passes_test(lambda u: u.is_superuser)
def jury_export_web(request):
	jurys = Jury.objects.filter(team=None).order_by('surname')

	return render(request, 'FPT2018/listing_jurys_web.html', {'jurys': jurys})
################# SUPER USERS VIEWS #################
#####################################################



@user_passes_test(ninja_test, redirect_field_name=None, login_url='/FPT2018/soon')
@cache_page(cache_duration)
def participants_overview(request):
	...
	return render(request, 'FPT2018/participants_overview.html', {'participants': participants})
...
def participants_all(request):
	...
	return render(request, 'FPT2018/participants_all.html', {'participants': participants})
...
def jury_detail(request, pk):
	...
	mygrades = JuryGrade.objects.filter(jury=jury)
	return render(request, 'FPT2018/jury_detail.html', {'jury': jury, "grades": mygrades})
...
def tournament_overview(request):
	...
	return render(request, 'FPT2018/tournament_overview.html', {'teams': teams, 'rounds': rounds, 'pfs': pfs, 'roomnumbers':roomnumbers, 'orderedroundsperroom': orderedroundsperroom})
...
def teams_overview(request):
	teams = Team.objects.all()
	teams = sorted(teams, key=lambda team: team.name)
	return render(request, 'FPT2018/teams_overview.html', {'teams': teams})
...
def team_detail(request, team_name):
	...
	return render(request, 'FPT2018/team_detail.html', {'team': team, 'participants': rankedparticipants, 'teamleaders': teamleaders, 'allrounds': allrounds, 'penalties': penalties})
...
def problem_detail(request, pk):
	...
	return render(request, 'FPT2018/problem_detail.html', {'problem': problem, "meangrades": meangrades, "teamresults": teamresults})
...
def rounds(request):
	...
	if with_final_pf :
		...
		return render(request, 'FPT2018/rounds.html', {'orderedroundsperroom': orderedroundsperroom, 'finalrounds': finalrounds, "finalranking": finalranking})

	else :
		return render(request, 'FPT2018/rounds.html', {'orderedroundsperroom': orderedroundsperroom})
...
def round_detail(request, pk):
	...
	return render(request, 'FPT2018/round_detail.html', {'round': round, 'jurygrades': jurygrades, 'meangrades': meangrades, "tacticalrejections": tacticalrejections, "eternalrejection": eternalrejection, "started": started, "finished": finished})
...
def finalround_detail(request, pk):
	...
	return render(request, 'FPT2018/finalround_detail.html', {'round': round, 'jurygrades': jurygrades, 'meangrades': meangrades, "tacticalrejections": tacticalrejections, "eternalrejection": eternalrejection, "started": started, "finished": finished})
...
def physics_fights(request):
	...
	return render(request, 'FPT2018/physics_fights.html', {'pf1': pf1, 'pf2': pf2, 'pf3': pf3})
...
def physics_fight_detail(request, pfid):
	...
	return render(request, 'FPT2018/physics_fight_detail.html', {"roomgrades": roomgrades})
...
def ranking(request):
	...
	return render(request, 'FPT2018/ranking.html', {'rankteams': rankteams})
