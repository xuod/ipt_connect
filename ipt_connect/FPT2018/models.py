...
class UploadToPathAndRename(object):
    ...
    def __call__(self, instance, filename):
        ...
        # get filename
        if instance.pk:
            filename = iri_to_uri(replace((u'{}_{}_{}.{}').format(
                instance.team, instance.surname, instance.name, ext), ' ', '_'))
        else:
...
class Participant(models.Model):
    ...
    GENDER_CHOICES = (('M', 'Homme'), ('F', 'Femme'),
                      ('D', 'Ne souhaite pas préciser'))

    ROLE_CHOICES = (('TM', 'Team Member'), ('TC', 'Team Captain'),
                    ('TL', 'Team Leader'), ('ACC', 'Accompagnant'))

    DIET_CHOICES = (('NO', 'No specific diet'), ('NOPORK', 'No pork'), ('NOMEAT',
                                                                        'No meat'), ('NOFISH', 'No fish'), ('NOMEAT_NOEGG', 'No meat, No eggs'))
    ...
    email = models.EmailField(
        help_text='Cette adresse sera utilisée pour envoyer des informations importantes aux participants.', verbose_name='Email')
    birthdate = models.DateField(
        default='1900-01-31', verbose_name='Date de naissance')
    photo = models.ImageField(upload_to=UploadToPathAndRename(
        'FPT2018/id_photo'), help_text="Merci d'utiliser une photo d'identité.", null=True)
    team = models.ForeignKey('Team', null=True, verbose_name='Équipe')
    role = models.CharField(max_length=20, choices=ROLE_CHOICES,
                            help_text="L'équipe doit comporter exactement un Team Captain (étudiant), entre deux et cinq Team Members (étudiants) et entre un et deux Team Leaders (encadrants). N'oubliez pas de vous ajouter vous-même !", default="TM", verbose_name='Rôle')
    #affiliation = models.CharField(max_length=50,default='XXX University')
    veteran = models.BooleanField(
        default=False, help_text="Est-ce que cette personne a déjà pris part au FPT ou à l'IPT ? (indicatif)", verbose_name='Vétéran')
    ...
    def update_scores(self):
        ...
        self.mean_score_as_reporter = self.tot_score_as_reporter / \
            max(len(rounds_as_reporter), 1)
        self.mean_score_as_opponent = self.tot_score_as_opponent / \
            max(len(rounds_as_opponent), 1)
    ...
    def fast_team_ranking(cls, team):
        participants = Participant.objects.filter(
            role='TM', team=team) | Participant.objects.filter(role='TC', team=team)
...
class Problem(models.Model):
    ...
    def status(self, verbose=True, meangradesonly=False):
        ...
        for round in rounds:
            if len(JuryGrade.objects.filter(round=round)) > 0:
                ...
                reporters.append({"name": round.reporter_team.name,
                                  "round": round, "value": round.score_reporter})
                opponents.append({"name": round.opponent_team.name,
                                  "round": round, "value": round.score_opponent})
                # reviewers.append({"name": round.reviewer_team.name,
                                  # "round": round, "value": round.score_reviewer})
        ...
        if 0 in [len(reporters), len(opponents), len(reviewers)]:
            ...
            meangrades = {"report": mean([reporter["value"] for reporter in reporters]), "opposition": mean(
                [opponent["value"] for opponent in opponents]), "review": mean([reviewer["value"] for reviewer in reviewers])}
    ...
    def update_scores(self):
        ...
        self.mean_score_of_reporters = mean(
            [round.score_reporter for round in rounds])
        self.mean_score_of_opponents = mean(
            [round.score_opponent for round in rounds])
        self.mean_score_of_reviewers = mean(
            [round.score_reviewer for round in rounds])
...
class Team(models.Model):
    ...
    IOC = models.OneToOneField(User, on_delete=models.SET_NULL, null=True,
                               blank=True, related_name='Team_FPT2018', verbose_name="Référent")
    ...
    def presentation_coefficients(self, verbose=False):
        ...
        eternalrejections = EternalRejection.objects.filter(
            round__reporter_team=self)

        beforetactical = []
        netrej = 0
        for pf in pfs:
            netrej += len(eternalrejections.filter(round__pf_number=pf))
            beforetactical.append(3.0 - reject_malus *
                                  max(0, (netrej - netreject_max)))
    ...
    #     rounds = Round.objects.filter(reporter_team=self) | Round.objects.filter(
    #         opponent_team=self) | Round.objects.filter(reviewer_team=self)
    #     # mypfnumbers = [1, 2, 3]
    #
    #     bonuspoints = []
    #     for mypfnumber in pfs:
    #         # for a given round, I am always in the same room
    #         pfrounds = rounds.filter(pf_number=mypfnumber)
    #
    ...
    #             pfrounds = [
    #                 round for round in rounds if round.pf_number == pfnumber]
    #         else:
    #             pass
    #
    #         assert len(pfrounds) <= maxround, "Team %s played %s rounds in Physics Fight %s. Check your database!" % (
    #             self.name, str(len(pfrounds)), str(mypfnumber))
    #         if len(pfrounds) == maxround:  # then all the fights are played
    #             teams = [pfrounds[0].reporter.team,
    #                      pfrounds[0].opponent.team, pfrounds[0].reviewer.team]
    #             if verbose:
    ...
    #                         pfnumber=mypfnumber, rounds=pfrounds, verbose=False)
    #                     for grade in average_grades:
    #                         if grade["role"] == "reporter":
    #                             teampfpoints += grade["value"] * \
    #                                 prescoeff[grade["round"].pf_number - 1]
    ...
    #
    #                 if verbose:
    #                     msg = "\t%i) Team %s -- %.2f points" % (
    #                         ind + 1, result["name"], result["points"])
    #                     if result["name"] == self.name:
    #                         print '\x1b[32m%s\x1b[0m' % msg
    #
    #                     else:
    ...
    #         average_grades = participant.compute_average_grades(
    #             pfnumber=pfnumber, rounds=rounds, verbose=verbose)
    ...
    #                 if verbose:
    #                     msg += '\n\t%.2f*%.1f = %.2f points as a reporter,' % (
    #                         grade["value"], pfprescoeff, grade["value"] * pfprescoeff)
    #             elif grade["role"] == "opponent":
    #                 points += grade["value"] * 2.0
    #                 if verbose:
    #                     msg += '\n\t%.2f*2 = %.2f points as an opponent,' % (
    #                         grade["value"], grade["value"] * 2)
    #             elif grade["role"] == "reviewer":
    #                 points += grade["value"]
    #                 if verbose:
    #                     msg += '\n\t%.2f points as a reviewer,' % (
    #                         grade["value"])
    #
    #             else:
    ...
    #     if bonuspoints:
    #         allpoints += sum(self.bonuspoints(pfnumber=pfnumber,
    ...
    #         pfnumber=pfnumber, rounds=rounds, verbose=verbose))[::-1]
    #     if verbose:
    #         print "=" * 20, "Team Ranking", "=" * 20
    #         for ind, team in enumerate(teams):
    #             if team == self and sys.stdout.isatty():
    #                 msg = str(ind + 1) + ") " + str(team.name) + " - " + str(
    #                     team.points(pfnumber=pfnumber, rounds=rounds, verbose=False)) + " points"
    #                 print '\x1b[32m%s\x1b[0m' % msg
    #             else:
    #                 msg = str(ind + 1) + ") " + str(team.name) + " - " + str(
    #                     team.points(pfnumber=pfnumber, rounds=rounds, verbose=False)) + " points"
    ...
    def problems(self, verbose=False, currentround=None):
        ...
        assert len(
            noproblems) == 3, "Something wrong with your rejected problem..."
        return noproblems
...
class Jury(models.Model):
    ...
    affiliation = models.CharField(
        max_length=100, blank=True, verbose_name='Affiliation à afficher')
    team = models.ForeignKey('Team', null=True, blank=True)
    pf1 = models.BooleanField(
        default=False, verbose_name='Présent lors du PF 1 ?')
    pf2 = models.BooleanField(
        default=False, verbose_name='Présent lors du PF 2 ?')
    pf3 = models.BooleanField(
        default=False, verbose_name='Présent lors du PF 3 ?')
    pf4 = models.BooleanField(
        default=False, verbose_name='Présent lors du PF 4 ?')
    remark = models.TextField(blank=True, verbose_name='Remarques')
...
class Round(models.Model):
    ...
    pf_number = models.IntegerField(
        choices=(((ind + 1, 'Fight ' + str(ind + 1))
                  for ind in range(npf_tot))),
        default=None
    )
    ...
    reporter_2 = models.ForeignKey(
        Participant, blank=True, null=True, related_name='reporter_name_2')
    ...
    def unavailable_problems(self, verbose=False):
        ...
        reporter_problems = self.reporter_team.problems(
            verbose=False, currentround=self)
        opponent_problems = self.opponent_team.problems(
            verbose=False, currentround=self)
        ...
        thispfrounds = Round.objects.filter(
            pf_number=self.pf_number).filter(room=self.room)
        presented_this_pf = [
            round.problem_presented for round in thispfrounds if round.round_number < self.round_number]
        if verbose:
...
def update_points(sender, instance, **kwargs):
    ...
    if (instance.reporter_team is None) or (instance.opponent_team is None) or (instance.reviewer_team is None) or instance.problem_presented is None:
        ...
        teams = [instance.reporter_team,
                 instance.opponent_team, instance.reviewer_team]
